...
describe("Tests PizzaUni", function() {
    describe("test constructor", function() {
        ...
        it("test registerUsers", function() {
            let pizzaUni = new PizzUni;

            assert.deepEqual(pizzaUni.registeredUsers, []);
        });

        it ('availableProducts', () => {
            let pizzaUni = new PizzUni;
            assert.deepEqual(pizzaUni.availableProducts.pizzas, ["Italian Style", "Barbeque Classic", "Classic Margherita"]);
        });
        it ('availableProducts', () => {
            let pizzaUni = new PizzUni;
            assert.deepEqual(pizzaUni.availableProducts.drinks, ['Coca-Cola', 'Fanta', 'Water']);
        });

        it ('checking orders', () => {
            let pizzaUni = new PizzUni;
            assert.deepEqual(pizzaUni.orders, []);
        });
     });
     ...
     describe('registerUser email', () => {
        ...
        it('registerUser email', () => {
            let pizzaUni = new PizzUni;
            assert.equal(pizzaUni.registerUser('str'), `This email address email is already being used!`);
         });
